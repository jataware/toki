"""Internal tool-call envelope parser.

Specialized push-fed parser for one top-level JSON object (the args dict of a
streaming tool call). Yields tagged events as input is fed via `feed()`:

  - top-level string values stream as post-decoded characters via `arg_chunk`
  - other top-level value types (numbers, booleans, null, arrays, objects)
    stream as raw JSON text via `arg_chunk` (so consumers can pipe nested
    structures into the public `streaming_parse_json` if they want a recursive
    view)
  - the parsed Python value of each arg is delivered in `arg_end`
